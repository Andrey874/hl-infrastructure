#!/bin/bash
function sqlQuery() {
  psql postgres://postgres:postgres@10.0.0.10:5001/userdata -c "${1}"
}
dbuserid=$(psql -At postgres://postgres:postgres@10.0.0.10:5001/userdata -c "select userid from users where username='guru';")
 ...
 while getopts "ld:p:" opt; do
   case "$opt" in
	...
	l)
	 ...
	 ;;
	p)
	  sqlQuery "SELECT data FROM pastedata WHERE dataid='${OPTARG}' 
	            AND userid='${dbuserid}';" | base64 -id
	  ;;
   esac
  done
fi
